  ...
  if(bOptics) {
	...
	if(beta>1./1.49 && theta>0 && theta<90) {
	  if(particleType==G4Electron::ElectronDefinition() ||
	    ...
	    nphoton2[izdc]=nphoton2[izdc]+felectron->Interpolate(theta,beta)
	      *fabs(particleType->GetPDGCharge());
	  }else{
	    nphoton2[izdc]=nphoton2[izdc]+fmuon->Interpolate(theta,beta)
	      *fabs(particleType->GetPDGCharge());
	  }
	}
      }
...
void RHICfZDCSD::SetTables(G4String atables)
{
  /// Read table for optical photon calculation (response function)
  fdata=new TFile((atables+"/ZDClightyield.root").c_str());
